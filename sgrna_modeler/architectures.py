...
def build_kim2018_comb(input_shape=(34,4)):
    Input_SEQ = k.layers.Input(shape=input_shape)
    C1 = k.layers.Convolution1D(80, 5, activation='relu')(Input_SEQ)
    P1 = k.layers.AveragePooling1D(2)(C1)
    F = k.layers.Flatten()(P1)
    ...
    D3 = k.layers.Dense(40, activation='relu')(DO3)
    DO4 = k.layers.Dropout(0.3)(D3)
    Old_Output = k.layers.Dense(1, activation='linear')(DO4)
    Modifier = k.layers.Input(shape = (0,))
    Merged = k.layers.Concatenate([Modifier, Old_Output])
    Norm = k.layers.Dense(10)(Merged, activation = 'relu')
    Output = k.layers.Dense(1, activation='linear')(Norm)
    model = k.models.Model(inputs=[Input_SEQ, Modifier], outputs=Output)
